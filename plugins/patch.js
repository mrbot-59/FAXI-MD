  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`FAXI-MD- UPDATE WAS SUCCESSFULL ✅`");
      } else {
    ...
    try {
      ...
      const designs = [
        async () => {
          ...
          const captionText = `FAXI-MD 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\nFAXI-MD`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*FAXI-MD ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "FAXI-MD Running Since",
        body: message,
        footerText: "FAXI-MD 2024",
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("FAXI-MD_FANCY_TEXT")
        ) {
